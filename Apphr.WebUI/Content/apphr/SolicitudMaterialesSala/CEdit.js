...
$(function () {
    ...
    master.id1 = $('#SolicitudMaterialSalaId');
    ...
    master.saveSuccess = function (response) {
        if (response.success) {
            if (master.isInsert()) {                
                master.id1.val(response.data.SolicitudMaterialSalaId);
    ...
    master.initForm = function () {
        $('#Fecha, #DespachoInventarioId, #PacienteNombreCompleto').attr('readonly', 'readonly').attr('tabindex','-1');
        if (master.isInsert()) {
...
//    };
    ...
    child.clearForm = function () {
        ...
        $('#SolicitudMaterialSalaDetalleId').val('0');
        $('#ProveedorId').val('');
        $('#MaterialId').val('');
        $('#PacienteId').val('');
        // Showing
...
//    });
    ...
    $('#Cantidad').on('input', () => {
        let newExistence = $('#MaterialExistencia').val() - $('#Cantidad').val();
        if (newExistence <= $('#MaterialMinimo').val()) {
            $('#invInfo').show();
        } else {
            $('#invInfo').hide();
        }
